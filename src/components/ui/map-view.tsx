...
export function MapView({
  ...
  const getCityCoordinates = async (city: string, customCityName?: string, address?: string) => {
    ...
    if (city !== 'custom' && cityCoordinates[city]) {
      // Если у нас есть адрес, пытаемся геокодировать более точное местоположение
      if (address && window.google) {
        try {
          const geocoder = new window.google.maps.Geocoder()
          const fullAddress = `${cities.find(c => c.value === city)?.label || city}, ${address}`
          ...
          // В случае ошибки используем координаты города
        }
      }
    ...
    if (city === 'custom' && customCityName && window.google) {
      try {
        ...
        // Полный адрес с учетом конкретного адреса, если он указан
        const fullAddress = address ? `${customCityName}, ${address}` : customCityName

        // Получение координат по названию города или полному адресу
        const response = await new Promise<any[]>((resolve, reject) => {
          geocoder.geocode({ address: fullAddress }, (results: any, status: any) => {
            if (status === 'OK' && results) {
              resolve(results)
